...
<template>
    <button v-if="!cars.map(car=>car.driver).includes(user.id)" type="button" class="btn btn-primary">Add Car</button>
    ...
    <table class="table">
        ...
        <tbody>
            <CarRow v-for="(car, index) in cars" :eventId="eventId" :car="car" :userIsDriver="cars.map(car=>car.driver).includes(user.id)"/>
        </tbody>
    </table>
</template>
